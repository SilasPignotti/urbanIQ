...
from app.services.nlp_service import NLPService, ParsedRequest, BERLIN_DISTRICTS, AVAILABLE_DATASETS
...
class TestNLPServiceIntegration:
    ...
    def test_nlp_service_job_model_integration(self, mock_llm_class):
        ...
        job = Job(
            request_text="Pankow Gebäude für Stadtplanung",
            **job_data
        )
...
class TestNLPServiceRealAPI:
    ...
    def test_real_api_german_parsing(self):
        ...
        for case in test_cases:
            ...
            assert set(result.datasets) == set(case["expected_datasets"]), f"Failed for: {case['input']}"
            assert result.confidence > 0.7, f"Low confidence ({result.confidence}) for: {case['input']}"
