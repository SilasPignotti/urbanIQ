...
class TestOverpassConnector:
    ...
    @pytest.mark.asyncio
    async def test_fetch_transport_stops_success(self, connector, sample_district_boundary, sample_overpass_response):
    ...
    @pytest.mark.asyncio
    async def test_fetch_transport_stops_json_decode_error(self, connector, sample_district_boundary):
    ...
    async def test_fetch_transport_stops_http_errors(self, connector, sample_district_boundary):
        ...
        with patch.object(connector, '_make_request', side_effect=RateLimitError("Rate limited")):
            with pytest.raises(RateLimitError):
                await connector.fetch_transport_stops(sample_district_boundary)
        
        # Test service unavailable error
        with patch.object(connector, '_make_request', side_effect=ServiceUnavailableError("Service down")):
            with pytest.raises(ServiceUnavailableError):
                await connector.fetch_transport_stops(sample_district_boundary)

    @pytest.mark.asyncio
    async def test_fetch_transport_stops_crs_transformation(self, connector, sample_overpass_response):
